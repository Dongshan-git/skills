...
Every Agent call must explicitly specify the selected type and matching model. Never rely on model inheritance. Keep effort and turn caps in the role definition, and read the resolved model from the Agent tool result.
