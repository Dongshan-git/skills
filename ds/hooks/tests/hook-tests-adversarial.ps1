...
Invoke-Hook 'D1b backtick regex + compliant calls' (Wf "const fence = /``/g;`nconst notes = await agent('Summarise', { model: 'sonnet' });`nconst c = await agent('Check', { model: 'opus' });") 'allow'
Invoke-Hook 'D2 quote in regex same line' (Wf "const re = /don't/; await agent('p', { model: 'fable' });") 'deny'
Invoke-Hook 'D2b quote in regex + compliant' (Wf "const re = /say`"/; await agent('p', { model: 'haiku' });") 'allow'
Invoke-Hook 'D2c division not regex' (Wf "const n = 10 / 2; const m = a / b / c; await agent('p', { model: 'sonnet' });") 'allow'
Invoke-Hook 'D2d regex with class containing slash' (Wf "const re = /[/]x/; await agent('p', { model: 'sonnet' });") 'allow'
...
Invoke-Hook 'D8 quoted key single' (Wf "await agent('p', { 'model': 'sonnet' });") 'allow'
Invoke-Hook 'D8b quoted key double' (Wf "await agent('p', { `"model`": `"sonnet`" });") 'allow'
Invoke-Hook 'D8c quoted agentType key' (Wf "await agent('p', { 'agentType': 'qa' });") 'allow'
Invoke-Hook 'D8d prompt string containing model: text at depth 1' (Wf "await agent('p', { prompt: 'model: fable is bad', model: 'sonnet' });") 'allow'
...
Invoke-Hook 'O nested object with model inside opts' (Wf "await agent('p', { schema: { properties: { model: { type: 'string' } } }, model: 'sonnet' });") 'allow'
Invoke-Hook 'O2 comment inside args' (Wf "await agent('p', /* opts */ { model: 'sonnet' /* fine */ });") 'allow'
Invoke-Hook 'O3 CRLF input' (Wf "const a = await agent('p1', { model: 'sonnet' });`r`nconst b = await agent('p2', { model: 'haiku' });`r`n") 'allow'
Invoke-Hook 'O4 template with regex-looking text' (Wf "await agent(``Use /grep/ and a/b ratio``, { model: 'sonnet' });") 'allow'
Invoke-Hook 'O5 model in template interpolation ignored' (Wf "const P='x'; await agent(``Prefix `${P} `${JSON.stringify({model:'fable'})}``, { model: 'haiku' });") 'allow'
Invoke-Hook 'O6 arrow in pipeline with regex in code' (Wf "const clean = s => s.replace(/``/g, '');`nconst r = await pipeline(ITEMS, it => agent(``Do `${clean(it)}``, { model: 'haiku' }), (p, it) => agent('verify', { model: 'sonnet' }));") 'allow'
Invoke-Hook 'O7 model then trailing comma' (Wf "await agent('p', { model: 'sonnet', });") 'allow'
...
Invoke-Hook 'E3 Chinese prompt in workflow script' (Wf "await agent('核对第十一节的检查清单', { model: 'sonnet' });") 'allow'
Invoke-Hook 'E4 emoji in workflow prompt' (Wf "await agent('Check 🚀 rollout', { model: 'haiku' });") 'allow'
...
Invoke-Hook 'N1b postfix -- before slash count' (Wf "const r = x-- / n; await agent('a', { model: 'sonnet' });`nawait agent('b', { model: 'sonnet' });") 'allow'
...
Invoke-Hook 'N4 20KB depth-1 option string performance' (Wf ("await agent('p', { model: 'sonnet', label: '" + ('x' * 20000) + "' });")) 'allow'
Invoke-Hook 'F1 property named agent' (Wf "const out = [];`nconst r = await agent('p', { model: 'sonnet' });`nout.push({ agent: 'p', result: r });") 'allow'
Invoke-Hook 'F2 reading field named agent' (Wf "const r = await agent('p', { model: 'sonnet' });`nconsole.log(r.agent);") 'allow'
...
Invoke-Hook 'S1b property key after comma still ask' (Wf "const r = await agent('p', { model: 'sonnet' });`nout.push({ id: 1, agent: 'p', result: r });") 'allow'
...
Invoke-Hook 'S3g field read on result still ask' (Wf "const r = await agent('p', { model: 'sonnet' });`nconsole.log(r.agent, r.workflow);") 'allow'
Invoke-Hook 'S3h shorthand property denied' (Wf "const r = await agent('p', { model: 'sonnet' }); out.push({ agent });") 'deny'
Invoke-Hook 'O8 real research script' (@{ tool_name = 'Workflow'; tool_input = @{ scriptPath = 'C:\Users\DS\.claude\projects\E--ds-docs\f2fc05d6-e4ab-4142-a95b-4b645975a631\workflows\scripts\claude-code-deep-research-2-1-267-wf_902195b8-125.js' }; cwd = 'E:\ds-docs' } | ConvertTo-Json -Compress) 'allow'
